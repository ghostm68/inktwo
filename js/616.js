...
// UI Selectors
const activateBtn = document.getElementById('activate-core');
const onboardingUI = document.getElementById('neural-onboarding');
const activeUI = document.getElementById('ai-active-interface');
...
activateBtn.onclick = async () => {
    try {
        ...
        activateBtn.textContent = "SYNCING_CORE...";
        addTicker("PROBING_GPU...");

        engine = await webllm.CreateMLCEngine(modelId, {
            initProgressCallback: (report) => {
                const p = Math.round(report.progress * 100);
                addTicker(`DL_WEIGHTS: ${p}%`);
                
                if (p === 100) {
                    // THE SWITCH: Hide onboarding, show chat
                    onboardingUI.style.display = 'none';
                    activeUI.style.display = 'flex';
                    addTicker("NEURAL_CORE_ONLINE");
                }
            }
        });
        ...
        aiInput.onkeypress = async (e) => {
            if (e.key === 'Enter') {
                ...
                // User Echo
                aiMessages.innerHTML += `<div style="color: #555; margin-top: 10px;">> ${msg}</div>`;
                aiInput.value = '';

                // AI Response
                const botMsg = document.createElement('div');
                botMsg.style.marginTop = "5px";
                botMsg.textContent = "◈ ";
                aiMessages.appendChild(botMsg);

                const chunks = await engine.chat.completions.create({
                    messages: [
                        { role: "system", content: "TONE: BURROUGHSIAN. SHORT DATA." },
                ...
                for await (const chunk of chunks) {
                    botMsg.textContent += chunk.choices[0]?.delta?.content || "";
        ...
        addTicker(`FATAL: ${err.message}`);
        activateBtn.textContent = "CORE_FAILED";
    }
};
